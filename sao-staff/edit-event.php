...
if ($result->num_rows == 0) {
    set_message('danger', 'Event not found or you do not have permission to edit it');
    header("Location: manage-events.php");
    exit();
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Handle Image Upload
    $event_image = $event['event_image']; // Keep existing image
    if (isset($_FILES['event_image']) && $_FILES['event_image']['error'] == 0) {
        $upload_result = upload_file($_FILES['event_image'], '../assets/images/', ['jpg', 'jpeg', 'png', 'gif']);
        
        if ($upload_result['success']) {
            // Delete old image if not default
            if ($event['event_image'] != 'default-event.jpg' && file_exists('../assets/images/' . $event['event_image'])) {
                unlink('../assets/images/' . $event['event_image']);
            }
    ...
    if (empty($errors)) {
        $stmt = $conn->prepare("UPDATE events SET event_title = ?, event_description = ?, event_category = ?, event_venue = ?, event_date = ?, event_time = ?, event_end_time = ?, event_image = ?, max_participants = ?, registration_deadline = ?, is_published = ? WHERE event_id = ? AND created_by = ?");
        $stmt->bind_param("ssssssssissii", $event_title, $event_description, $event_category, $event_venue, $event_date, $event_time, $event_end_time, $event_image, $max_participants, $registration_deadline, $is_published, $event_id, $user_id);
        
        if ($stmt->execute()) {
            log_activity($conn, $user_id, 'Event Updated', 'Updated event: ' . $event_title);
    ...
    // Update event array with posted values for form
...
$reg_count_query = "SELECT COUNT(*) as count FROM event_registrations WHERE event_id = ?";
$stmt = $conn->prepare($reg_count_query);
...
<head>
    ...
    <title>Edit Event - SAO Staff</title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
<body>
    <nav class="navbar">
        <div class="nav-container">
            ...
            <span class="page-title-nav">Edit Event</span>
        </div>
    </nav>
    ...
    <div class="main-container">
        <div class="page-header">
            ...
            <p class="page-subtitle">Update event details and settings</p>
        </div>

        <?php if ($reg_count > 0): ?>
            <div class="info-banner">
                <span class="info-text">
                    ⚠️ This event has <?php echo $reg_count; ?> registration(s). Changes may affect registered students.
                </span>
            </div>
        <?php endif; ?>
        ...
        <form method="POST" enctype="multipart/form-data">
            <div class="form-card">
                ...
                <div class="form-section">
                    ...
                    <div class="form-group">
                        ...
                        <textarea name="event_description" class="form-textarea" 
                                  placeholder="Describe the event in detail" required><?php echo htmlspecialchars($event['event_description']); ?></textarea>
                    ...
                    <div class="form-grid">
                        <div class="form-group">
                            ...
                            <input type="text" name="event_venue" class="form-input" 
                                   ...
                                   value="<?php echo htmlspecialchars($event['event_venue']); ?>">
                        </div>
                ...
                <div class="form-section">
                    ...
                    <div class="form-group">
                        <?php if ($event['event_image'] && file_exists('../assets/images/' . $event['event_image'])): ?>
                            <div class="current-image">
                                <span class="current-image-label">Current Image:</span>
                                <img src="../assets/images/<?php echo htmlspecialchars($event['event_image']); ?>" alt="Current Event Image">
                            </div>
                        <?php endif; ?>

                        <div class="file-upload-area" onclick="document.getElementById('event_image').click()">
                            <div class="upload-icon">📸</div>
                            <p class="upload-text">Click to upload new event image (optional)</p>
                ...
                <div class="form-section">
                    ...
                    <div class="checkbox-group">
                        <input type="checkbox" name="is_published" id="is_published" class="checkbox-input" value="1"
                               <?php echo $event['is_published'] ? 'checked' : ''; ?>>
                        <label for="is_published" class="checkbox-label">
                            Publish this event (students will be able to see and register)
                        </label>
                    </div>
                </div>
